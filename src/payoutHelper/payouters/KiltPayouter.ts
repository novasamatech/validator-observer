...
export class KiltPayoutHelper extends PayoutHelper {
    ...
    private async needIncrement(validator: string): Promise<boolean> {
        ...
        const delegatorState = await this.retryApiCall(() => 
            this.api.query.parachainStaking.delegatorState(validator)
        );
        ...
        const blockRewarded = await this.retryApiCall(() => 
            this.api.query.parachainStaking.blocksRewarded(owner)
        );
        console.log(`Blocks rewarded for owner ${owner}: ${blockRewarded}`);

        const blockAuthored = await this.retryApiCall(() => 
            this.api.query.parachainStaking.blocksAuthored(owner)
        );
    ...
    private async needClaim(validator: string): Promise<boolean> {
        ...
        const rewardsToClaim = await this.retryApiCall(() => 
            this.api.query.parachainStaking.rewards(validator)
        );
